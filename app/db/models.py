...
class SessionSet(Base):
    ...
    # AI proposal stamped at log time (week_plan slot or live_set GF_NEXT)
    planned_kg: Mapped[float | None] = mapped_column(Float, nullable=True)
    planned_reps: Mapped[int | None] = mapped_column(Integer, nullable=True)
    planned_rpe: Mapped[int | None] = mapped_column(Integer, nullable=True)
    plan_source: Mapped[str | None] = mapped_column(String(16), nullable=True)  # week_plan | live
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class UserActionLog(Base):
    """High-signal bot actions for admin per-user activity trail."""

    __tablename__ = "user_action_logs"
    __table_args__ = (Index("ix_user_action_logs_user_id_id", "user_id", "id"),)
